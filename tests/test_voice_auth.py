...
class TestSelectBest:
    """VoiceAuth.select_best_voice のテスト。"""
    ...
    def test_single_sample(
        ...
        voices = [_to_voice_input(auth.extract_audio(audio))]
        selected, index = auth.select_best_voice(voices)
        assert index == 0
        assert selected is voices[0]
        np.testing.assert_array_equal(selected["embedding_values"], [1.0, 0.0, 0.0])
    ...
    def test_multiple_samples_selects_medoid(
        ...
        voices = [_to_voice_input(auth.extract_audio(audio)) for _ in range(3)]
        selected, index = auth.select_best_voice(voices)
        assert index in (0, 1)
    ...
    def test_empty_raises(self, auth: VoiceAuth) -> None:
        """空リストで ValueError。"""
        with pytest.raises(ValueError, match="voices が空です"):
    ...
    def test_extract_no_speech_raises(
        ...
        with pytest.raises(EmptyAudioError):
            auth.extract_audio(audio)

    @patch("voice_auth_engine.voice_auth.load_audio")
    @patch("voice_auth_engine.voice_auth.extract_speech")
    @patch("voice_auth_engine.voice_auth.detect_speech")
    @patch("voice_auth_engine.voice_auth.extract_embedding")
    def test_returns_voice_input_and_index(
        self,
        mock_extract_emb: MagicMock,
        mock_detect: MagicMock,
        mock_extract_sp: MagicMock,
        ...
        """select_best_voice() が tuple[VoiceInput, int] を返す。"""
        audio = make_audio(1.0)
        mock_load.return_value = audio
        mock_detect.return_value = make_segments(audio)
        mock_extract_sp.return_value = audio
        ...
        voices = [_to_voice_input(auth.extract_audio(audio))]
        selected, index = auth.select_best_voice(voices)
        assert "embedding_values" in selected
        assert "phoneme_values" in selected
        assert isinstance(index, int)


class TestVerify:
    """VoiceAuth.verify のテスト。"""
    ...
    def test_verify_rejected(self, auth: VoiceAuth) -> None:
        ...
        target: VoiceInput = {
            "embedding_values": list(make_embedding([0.0, 1.0, 0.0]).values),
            "phoneme_values": [],
        }
...
class TestSelectBestPhonemes:
    """select_best_voice の音素整合性チェックのテスト。"""
    ...
    @patch("voice_auth_engine.voice_auth.detect_speech")
    @patch("voice_auth_engine.voice_auth.load_audio")
    def test_extract_returns_phonemes(
        self,
        mock_load: MagicMock,
        mock_detect: MagicMock,
        mock_extract_sp: MagicMock,
        ...
        result = auth_with_phonemes.extract_audio(audio)
        assert result["phoneme_values"] == phonemes
        assert isinstance(result["embedding_values"], list)
    ...
    def test_selects_medoid_by_embedding(
        ...
        """select_best_voice() が embedding の medoid でサンプルを選択する。"""
        ...
        voices = [_to_voice_input(auth_with_phonemes.extract_audio(audio)) for _ in range(3)]
        selected, index = auth_with_phonemes.select_best_voice(voices)
        assert index in (0, 1)
        assert selected is voices[index]
    ...
    def test_raises_on_inconsistent_phonemes(
        ...
        voices = [_to_voice_input(auth.extract_audio(audio)) for _ in range(2)]
        with pytest.raises(PhonemeConsistencyError, match="音素列の不整合"):
            auth.select_best_voice(voices)
...
class TestVerifyPhonemes:
    """verify の音素照合ゲート付きテスト。"""
...
class TestExtract:
    ...
    def test_result_contains_all_fields(
        ...
        result = auth.extract_audio(original_audio)
